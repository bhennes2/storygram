...
SimileAjax.Graphics.createBubbleForContentAndPoint=function(F,E,C,B,D,A){
	...
	window.setTimeout(function(){
		...
		else{F.style.width=H+"px";I.content.appendChild(F)}
	},200)};
...
SimileAjax.Graphics.createBubbleForPoint=function(B,A,J,N,F){
	...
	var G=function(T){var S=document.createElement("div");S.className=K(E.borderGraphicCSSClassPrefix+T);D.appendChild(S)};
	G("top-left");
	G("top-right");
	G("bottom-left");
	G("bottom-right");
	...
	SimileAjax.History.addAction=function(A){
		...
		window.setTimeout(function(){
				...
				if(SimileAjax.WindowManager._draggedElement!=null){
					...
					if(!SimileAjax.WindowManager._dragging){
						if(Math.abs(J)>5||Math.abs(I)>5){
							try{if("onDragStart" in L){L.onDragStart()}
								if("ghost" in L&&L.ghost){
									...
									document.body.appendChild(K);L._ghostElmt=K
								}
			...
			Timeline._Band=function(B,G,C){
				...
				var A=this;this._syncWithBand=null;
				...
				var F=this._theme!=null?this._theme.mouseWheel:"zoom";
				...
				if(this._eventSource){
					this._eventListener={onAddMany:function(){A._onAddMany()},onClear:function(){A._onClear()}};this._eventSource.addListener(this._eventListener)}this._eventPainter=G.eventPainter;this._eventTracksNeeded=0;this._eventTrackIncrement=0;G.eventPainter.initialize(this,B);this._decorators=("decorators" in G)?G.decorators:[];for(var D=0;D<this._decorators.length;D++){this._decorators[D].initialize(this,B)}};Timeline._Band.SCROLL_MULTIPLES=5;
...
Timeline._Band.prototype.removeOnScrollListener=function(B){for(var A=0;A<this._onScrollListeners.length;A++){if(this._onScrollListeners[A]==B){this._onScrollListeners.splice(A,1);break}}};
Timeline._Band.prototype.setSyncWithBand=function(B,A){
	if(this._syncWithBand){this._syncWithBand.removeOnScrollListener(this._syncWithBandHandler)}this._syncWithBand=B;this._syncWithBand.addOnScrollListener(this._syncWithBandHandler);this._highlight=A;this._positionHighlight()};
...
Timeline._Band.prototype.zoom=function(C,A,F,E){
	if(!this._zoomSteps){return}A+=this._viewOffset;
	...
	/*Determine current interval unit being displayed 
	var time = ["MILLISECOND", "SECOND", "MINUTE", "HOUR", "DAY", "WEEK", "MONTH", "YEAR", "DECADE", "CENTURY"];
	var intervalUnit = this._zoomSteps[this._zoomIndex].unit;
	
	var min_date = this.getMinVisibleDate();
	var max_date = this.getMaxVisibleDate();
	
	/*document.getElementById("min_date").innerHTML = min_date;
	document.getElementById("max_date").innerHTML = max_date;
	document.getElementById("unit_date").innerHTML = time[intervalUnit];*/
...
Timeline._Band.prototype._onMouseMove=function(D,A,E){
	if(this._dragging){
		...
		document.getElementById("min_date").innerHTML = date_format(min_date);
		document.getElementById("max_date").innerHTML = date_format(max_date);
...
Timeline._Band.prototype._onMouseUp=function(B,A,C){
	this._dragging=false;this._keyboardInput.focus()
};
...
Timeline._Band.prototype._onMouseScroll=function(G,H,B){
	...
	Timeline._Band.prototype._moveEther=function(A){
		...
		else{this.softLayout()}this._onChanging()
		
		var min_date = this._timeline.getBand(0).getMinVisibleDate();
		var max_date = this._timeline.getBand(0).getMaxVisibleDate();
...
Timeline.CompactEventPainter.prototype.initialize=function(B,A){this._band=B;this._timeline=A;this._backLayer=null;this._eventLayer=null;this._lineLayer=null;this._highlightLayer=null;this._eventIdToElmt=null};
Timeline.CompactEventPainter.prototype.addOnSelectListener=function(A){this._onSelectListeners.push(A)};
Timeline.CompactEventPainter.prototype.removeOnSelectListener=function(B){for(var A=0;A<this._onSelectListeners.length;A++){if(this._onSelectListeners[A]==B){this._onSelectListeners.splice(A,1);break}}};
...
Timeline.CompactEventPainter.prototype.paint=function(){var N=this._band.getEventSource();
	...
	Timeline.CompactEventPainter.prototype._onClickMultiplePreciseInstantEvent=function(E,A,B){
		...
		var D=[];
		for(var C=0;C<B.length;C++){D.push(B[C].getID())}
	...
	Timeline.CompactEventPainter.prototype._onClickDurationEvent=function(F,B,C){
		if("pageX" in B){var A=B.pageX;var E=B.pageY}
		else{var D=SimileAjax.DOM.getPageCoordinates(F);var A=B.offsetX+D.left;var E=B.offsetY+D.top}
		this._showBubble(A,E,[C]);
		this._fireOnSelect([C.getID()]);B.cancelBubble=true;SimileAjax.DOM.cancelEvent(B);return false};
...
Timeline.CompactEventPainter.prototype._showBubble=function(A,F,B){
		...
		for(var D=0;D<B.length;D++){
			var C=document.createElement("div");
			E.appendChild(C);
			var pt = B[D].fillInfoBubble(C,this._params.theme, this._band.getLabeller())
		}
...
Timeline.GregorianEtherPainter.prototype.initialize=function(C,B){
	...
	this._markerLayer=null;this._lineLayer=null;
...
fillInfoBubble:function(A,E,M){
	...
	{				
		if ( timeline_data.events[i].image == B)
		{ 
			break;
		}
	}
...
function scroll_right(A){
	
	var myInterval = false;
	$('.arrow_right').mouseover(function(){
       myInterval = setInterval(function(){
           A.getBand(0)._moveEther(5);
       }, 50);
	});
...
function date_format(A){
	...
	var new_date = month+" "+day+", "+year+" "+hour+":"+minute+" "+time_of_day;
	return new_date;
}

function validate_input(){
	
	/* Variable used to test alphanumeric input */
	var input_text=/[^a-z0-9A-z]/i;
	
	/* Don't allow entry of blank in text area */
	if( document.forms["submit_form"]["input_text"].value == "" ){  
		document.getElementById("searchbar_notes").innerHTML ="Must enter a value";
		document.getElementById("input_text").focus();
		return false;
	}
	/* Don't allow entry without selecting search type */
	if( document.forms["submit_form"]["type_user"].checked == false && document.forms["submit_form"]["type_tag"].checked == false ){
		document.getElementById("searchbar_notes").innerHTML ="Must select type of search";
		document.getElementById("type_user").focus();
		document.getElementById("type_tag").focus();
		return false;
	}
	/* Don't allow entry of space in text area */
	if( document.forms["submit_form"]["input_text"].value.indexOf(' ') >= 0 ) {  
		document.getElementById("searchbar_notes").innerHTML ="Do not include spaces in search";
		return false;
	}
	/* Don't allow non-alphanumeric characters */
	if ( input_text.test(document.forms["submit_form"]["input_text"].value) ){  
		document.getElementById("searchbar_notes").innerHTML ="Only use alphanumeric characters";
		return false;
	}
